...
local bettingMonitorName = "back"
local bettingMonitor = peripheral.wrap(bettingMonitorName)
...
local paymentBlock = {-1, 2.1, 3} --the block the player has to stand on to pay
...
while true do
    if window=="idle" then
        ...
        parallel.waitForAny(drawWheelScreen, waitForPlayer)
    end
    if window=="betting" then
        ...
        function getBets()
            ...
            while true do
                ...
                elseif side == bettingMonitorName and bettingSum+10<=balance then
        ...
        function checkForPlayer()
            ...
            while true do
                ...
                for _, entity in pairs(data) do
                    if entity.key=="minecraft:player" and math.abs(entity.x-paymentBlock[1])<tolorance and math.abs(entity.y-paymentBlock[2])<tolorance and math.abs(entity.z-paymentBlock[3])<tolorance and entity.id==userId then
                        playerPresent = true
                    end
                end
        ...
        parallel.waitForAny(getBets, updateTable, checkForPlayer)
    ...
    if window=="spin" then
        ...
        local ballForce = -math.random()*5-3
